...
if (btc_db_load && {profileNamespace getVariable [format ["btc_hm_%1_db", worldName], false]}) then {
    if ((profileNamespace getVariable [format ["btc_hm_%1_version_moded_by_cetcaf", worldName], 1.13]) in [1.193, 1.2, btc_version select 1]) then {
...
{[_x, 3600] call btc_fnc_eh_veh_add_respawn;} forEach btc_helo;
...
addMissionEventHandler ['HandleDisconnect',{sleep 1;deleteVehicle (_this select 0);}];
addMissionEventHandler ['PlayerDisconnected',{sleep 1;deleteVehicle (_this select 0);}];
